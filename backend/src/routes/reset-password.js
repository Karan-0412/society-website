...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      .input('email', sql.VarChar, email)
      .input('token', sql.VarChar, hashedToken)
      .input('expiry', sql.DateTime, new Date(Date.now() + 3600000)) // 1 hour later
      .query(`
        UPDATE Users 
...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    const isValid = await bcryptjs.compare(token, user.resetToken);
    if (!isValid) {
